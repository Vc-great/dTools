	...
	if (isTauri()) {
		...
		const tauriInterval = useInterval(async () => {
			try {
				...
				if (monitor && monitor.scaleFactor) {
		...
		useEffect(() => {
			// 只有在窗口管理相关功能需要时才启动监听器
			if (WIN32_CUSTOM_TITLEBAR || osType === 'linux') {
				tauriInterval.start();
			}
			return () => {
				if (tauriInterval.active) {
					tauriInterval.stop();
				}
			};
		}, [osType]);
